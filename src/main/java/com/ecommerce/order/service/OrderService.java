...
public class OrderService {
    ...
    private OrderItem buildItem(OrderLineRequest line, String currency) {
        Product product = productService.getActive(line.productId());
        BigDecimal lineSubtotal = product.getPrice().multiply(BigDecimal.valueOf(line.quantity()));
        BigDecimal tax = money(lineSubtotal.multiply(taxRate));
        return new OrderItem(
                null, product.getId(), product.getSku(), product.getName(),
                product.getPrice(), line.quantity(), BigDecimal.ZERO, tax, currency
        );
    }
